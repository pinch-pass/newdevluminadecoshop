...
class ImageManagerCore
{
                                ...
                                &$src_width = null, &$src_height = null)
    {
		...
		if(Configuration::get('lumina_use_new_images')) { // alternatywa dla damidomo.ru
			if(strpos($dst_file, _PS_PROD_IMG_DIR_) !== false){
				if(strpos($dst_file, 'thickbox_default')){
					if($width_diff < 1 || $height_diff < 1) {
						if($width_diff < $height_diff){
							$next_width = $dst_width;
							$next_height = ceil($src_height * ($dst_width / $src_width));
						}else{
							$next_width = ceil($src_width * ($dst_height / $src_height));
					...
					if($width_diff > $height_diff) {
						$next_width = ceil($src_width * $width_diff);
						$next_height = ceil($src_height * $width_diff);
					} else {
						$next_width = ceil($src_width * $height_diff);
						$next_height = ceil($src_height * $height_diff);
					}
				}
			}else{
				if ($width_diff > 1 && $height_diff > 1) {
    ...
    public static function write($type, $resource, $filename)
    {
        ...
        switch ($type) {
            ...
            case 'png':
                ...
                $webpname = str_replace('.'.$type, '.webp', $filename);
            break;
