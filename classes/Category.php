...
* 	 * @param bool $method_to_create a category
    ...
    */
    ...
    public static function titlePage($titles){
        if (!empty(explode("=", $_SERVER['REQUEST_URI'])[1])){
            $categoryCode = explode("=", $_SERVER['REQUEST_URI'])[1];
        }
        else{
            $categoryCode = explode("=", $_SERVER['REQUEST_URI'])[0];
        }
    ...
    public static function replaceDesc($description, $title, $name){
        $description = str_ireplace(mb_strtolower($name), mb_strtolower($title), $description);
        return $description;
    }

    public static function descriptionSection(){
        if (!empty(explode("=", $_SERVER['REQUEST_URI'])[1])){
            $categoryCode = explode("-", explode("=", $_SERVER['REQUEST_URI'])[1])[1];
            return $categoryCode;
        }
        else{
            $categoryCode = explode("=", $_SERVER['REQUEST_URI'])[0];
    ...
    public function metaForSection()
    {
        $url = explode("/", $_SERVER['REQUEST_URI'])[1];
        $sql = 'SELECT url, title, meta_description, keywords, h1, description
		FROM `ps_category_meta`
		WHERE `url`= '.'"' . $url . '"';
