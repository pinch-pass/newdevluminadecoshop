    ...
    public static function getPath($id_category, $path = '', $link_on_the_item = false, $category_type = 'products', Context $context = null)
    {
        ...
        if ($category_type === 'products') {
            ...
            if ($path != $category->name) {
                ...
                $category_link = str_replace("/content/category/2-magaziny", "/magaziny", Tools::safeOutput($category_link));
                $full_path = ($link_on_the_item ? '<a href="'.$category_link.'" data-gg="">' : '').htmlentities($path, ENT_NOQUOTES, 'UTF-8').($link_on_the_item ? '</a>' : '');
            }
