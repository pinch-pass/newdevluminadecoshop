...
class SendpulseApi implements SendpulseApi_Interface {
     ...
     * @throws Exception
     */
    public function __construct( $userId, $secret, $storageType = 'file', $module_instanse, $type='false') {
        $this->mod = $module_instanse;
        $this->error_type = $type;
        if( empty( $userId ) || empty( $secret ) ) {

			if($type=='return'){
				$this->error = "Empty ID or SECRET";
				return array("api"=>false, "message"=>"Empty ID or SECRET");
			}else{
		    	exit(
		    		json_encode(array(
		    		"status"=>false, 
		    		"message"=>("Empty ID or SECRET")))
		    	);
        ...
        if( empty( $this->token ) ) {
            if( !$this->getToken() ) {
				if($type=='return'){
					$this->error = "Could not connect to api, check your ID and SECRET";
					return array("error"=>false, "message"=>"Could not connect to api, check your ID and SECRET");
				}else{            
		        	exit(
		        		json_encode(array(
		        		"status"=>false, 
		        		"message"=>("Could not connect to api, check your ID and SECRET")))
		        	);
		        }	
            }
        }
     ...
     * @return mixed|stdClass
     */
    public function getEmailsFromBook($id, $offset=0, $limit=100){
        if(empty($id)){
            return $this->handleError( 'Empty book id' );
        }
        $requestResult = $this->sendRequest( 'addressbooks/' . $id . '/emails', "GET", array("limit"=>$limit, "offset"=>$offset));
