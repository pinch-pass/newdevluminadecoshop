		...
		if (strpos($_SERVER['SCRIPT_FILENAME'], 'blocklayered-ajax.php') === false || Tools::getValue('selected_filters') !== false)
		{
			...
			if (!empty($url_attributes))
			{
				foreach ($url_attributes as $url_attribute)
				{
                    ...
                    if($this->l('price') === $attribute_name)
                        $attribute_name = 'price';
                    if($this->l('weight') === $attribute_name)
	...
	public function ajaxCall()
	{
		...
		$smarty->assign(
			array(
				...
				'products' => $products,
				'products_per_page' => isset($this->context->cookie->nb_item_per_page) ? (int)$this->context->cookie->nb_item_per_page : (int)Configuration::get('PS_PRODUCTS_PER_PAGE'),
