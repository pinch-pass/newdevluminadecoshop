...
<!-- Module Editorial -->
<div id="editorial_block_center" class="editorial_block container wow animated fadeInDown" style="visibility: visible;">
{*	{if $editorial->body_home_logo_link}<a href="{$editorial->body_home_logo_link|escape:'html':'UTF-8'}" title="{$editorial->body_title|escape:'html':'UTF-8'|stripslashes}">{/if}
	{if $homepage_logo}<img class="img-responsive" src="{$link->getMediaLink($image_path)|escape:'html'}" alt="{$editorial->body_title|escape:'html':'UTF-8'|stripslashes}" {if $image_width}width="{$image_width}"{/if} 
	{if $image_height}height="{$image_height}" {/if}/>{/if}
	...
	{if $editorial->body_paragraph}
		<div class="rte">{$editorial->body_paragraph|stripslashes}</div>
		<a id="more-infos">Развернуть</a>
	{/if}
</div>
<!-- /Module Editorial -->
