...
/*if (Configuration::get('PS_TOKEN_ENABLE') == 1 &&
	...
	$context->cookie->id_customer_wish === true
)
	echo $module->l('Invalid token', 'cart');
*/
if (!$context->cookie->id_customer_wish)
{
	$context->cookie->id_customer_wish = mt_rand(1, 2147483647);
}
if ($context->cookie->id_customer_wish)
{
	if ($id_wishlist && WishList::exists($id_wishlist, $context->cookie->id_customer_wish) === true)
		$context->cookie->id_wishlist = (int)$id_wishlist;

	if ((int)$context->cookie->id_wishlist > 0 && !WishList::exists($context->cookie->id_wishlist, $context->cookie->id_customer_wish))
	...
	if (($add || $delete) && empty($id_product) === false)
	{
		if (!isset($context->cookie->id_wishlist) || $context->cookie->id_wishlist == '')
		{
			...
			$wishlist->id_customer = (int)$context->cookie->id_customer_wish;
			list($us, $s) = explode(' ', microtime());
			srand($s * $us);
			$wishlist->token = strtoupper(substr(sha1(uniqid(rand(), true)._COOKIE_KEY_.$context->cookie->id_customer_wish), 0, 16));
			$wishlist->add();
			$context->cookie->id_wishlist = (int)$wishlist->id;
		}
		if ($add && $quantity)
			WishList::addProduct($context->cookie->id_wishlist, $context->cookie->id_customer_wish, $id_product, $id_product_attribute, $quantity);
		else if ($delete)
			WishList::removeProduct($context->cookie->id_wishlist, $context->cookie->id_customer_wish, $id_product, $id_product_attribute);
	}
	$context->smarty->assign('products', WishList::getProductByIdCustomer($context->cookie->id_wishlist, $context->cookie->id_customer_wish, $context->language->id, null, true));
