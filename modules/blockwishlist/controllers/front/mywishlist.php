...
class BlockWishListMyWishListModuleFrontController extends ModuleFrontController
{
	...
	public function assign()
	{
		...
		if ($this->context->cookie->id_customer_wish)
		{
			...
			if (Tools::isSubmit('submitWishlist'))
			{
				...
				if (!count($errors))
				{
					...
					if (!count($errors))
					{
						...
						$wishlist->name = $name;
						$wishlist->id_customer = (int)$this->context->cookie->id_customer_wish;
						!$wishlist->isDefault($wishlist->id_customer) ? $wishlist->default = 1 : '';
						list($us, $s) = explode(' ', microtime());
						srand($s * $us);
						$wishlist->token = strtoupper(substr(sha1(uniqid(rand(), true)._COOKIE_KEY_.$this->context->cookie->id_customer_wish), 0, 16));
						$wishlist->add();
						Mail::Send(
				...
				WishList::addCardToWishlist($this->context->cookie->id_customer_wish, Tools::getValue('id_wishlist'), $this->context->language->id);
			elseif ($delete && empty($id_wishlist) === false)
			{
				$wishlist = new WishList((int)$id_wishlist);
				if ($this->context->cookie->id_customer_wish && $this->context->cookie->id_customer_wish == $wishlist->id_customer && Validate::isLoadedObject($wishlist))
					$wishlist->delete();
				else
			...
			$this->context->smarty->assign('wishlists', WishList::getByIdCustomer($this->context->cookie->id_customer_wish));
			$this->context->smarty->assign('nbProducts', WishList::getInfosByIdCustomer($this->context->cookie->id_customer_wish));
		}
		else
		...
		$this->context->smarty->assign(array(
			'id_customer' => (int)$this->context->cookie->id_customer_wish,
			'errors' => $errors,
			'form_link' => $errors,
		));
	...
	public function ajaxProcessDeleteList()
	{
		...
		$wishlist = new WishList((int)$id_wishlist);
		if (Validate::isLoadedObject($wishlist) && $wishlist->id_customer == $this->context->cookie->id_customer_wish)
		{
	...
	public function ajaxProcessSetDefault()
	{
		...
		{
			$wishlist = new WishList((int)$id_wishlist);
			if (Validate::isLoadedObject($wishlist) && $wishlist->id_customer == $this->context->cookie->id_customer_wish && $wishlist->setDefault())
				die(Tools::jsonEncode(array('success' => true)));
		}
	...
	public function ajaxProcessProductChangeWishlist()
	{
		...
		if (!$id_product || !is_int($id_product_attribute) || !$quantity ||
			...
			(Validate::isLoadedObject($new_wishlist) && $new_wishlist->id_customer != $this->context->cookie->id_customer_wish) ||
			(Validate::isLoadedObject($old_wishlist) && $old_wishlist->id_customer != $this->context->cookie->id_customer_wish))
		...
		if ($check)
		{
			$res &= $old_wishlist->removeProduct($id_old_wishlist, $this->context->cookie->id_customer_wish, $id_product, $id_product_attribute);
			$res &= $new_wishlist->updateProduct($id_new_wishlist, $id_product, $id_product_attribute, $priority, $quantity + $check);
		}
		else
		{
			$res &= $old_wishlist->removeProduct($id_old_wishlist, $this->context->cookie->id_customer_wish, $id_product, $id_product_attribute);
			$res &= $new_wishlist->addProduct($id_new_wishlist, $this->context->cookie->id_customer_wish, $id_product, $id_product_attribute, $quantity);
		}
