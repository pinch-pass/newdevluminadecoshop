...
class OnePageCheckoutPSCore
{
    ...
    public function __construct($module, $context)
    {
        ...
        if (is_file($file_smarty_config) && $this->module->active) {
        ...
        if (!function_exists('curl_init')
            ...
            if (Tools::isSubmit('validate_license')) {
                ...
                if (Tools::getIsset('license_number') && !empty($params['license_number'])) {
                    ...
                    if (is_object($response)) {
                        if ($response->code == $this->CODE_ERROR) {
                            ...
                            if (version_compare(_PS_VERSION_, '1.5') >= 0) {
                                ...
                                Configuration::updateValue($this->module->prefix_module.'_RM', '1', $id_shop_group, $id_shop);
                            } else {
                    ...
                    $this->module->errors[] = $this->module->l('Please enter the license to do the validation of the module.', self::NAME_CLASS);
                }
            }
        }
    }
            ...
            foreach ($sub_dirs as $d) {
                if (is_dir($dir.'/'.$d)) {
                    ...
                    if (in_array($ext, array('tpl', 'css', 'js'))) {
                        if ($ext === 'js') {
                            $replace_url = _PS_THEME_DIR_.'js/modules/';
                        } elseif ($ext === 'tpl') {
                            $replace_url = _PS_THEME_DIR_.'modules/';
                        } elseif ($ext === 'css') {
                            $replace_url = _PS_THEME_DIR_.'css/modules/';
                        }

                        $module_file    = _PS_MODULE_DIR_.str_replace($replace_url, '', $dir).'/'.$d;
    ...
    public function searchOverrides()
    {
        ...
        $theme_module_dir = _PS_THEME_DIR_.'js/modules/'.$this->module->name.'/views/js/front';
        $this->searchThemeOverrides($theme_module_dir, $overrides);
    ...
    {
        if (!array_key_exists('JS_FILES', $this->module->params_back)) {
            $this->module->params_back['JS_FILES'] = array();
        }
        if (!array_key_exists('CSS_FILES', $this->module->params_back)) {
            $this->module->params_back['CSS_FILES'] = array();
        }

        //add anothers scripts
        if (version_compare(_PS_VERSION_, '1.6') < 0) {
            array_unshift(
                $this->module->params_back['JS_FILES'],
                $this->module->path.'views/js/lib/bootstrap/pts/bootstrap.min.js?v='.$this->module->version
            );
            ...
            if (version_compare(_PS_VERSION_, '1.5') < 0) {
                ...
                array_unshift(
                    $this->module->params_back['JS_FILES'],
                    $this->module->path.'views/js/lib/jquery/jquery.min/jquery.min.js?v='.$this->module->version
                );
            }
        ...
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/lib/jquery/plugins/growl/jquery.growl.css?v='.$this->module->version
        );
        array_push(
            $this->module->params_back['JS_FILES'],
        ...
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/lib/bootstrap/pts/pts-bootstrap.css?v='.$this->module->version
        );

        //switch
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/lib/simple-switch/simple-switch.css?v='.$this->module->version
        );
        ...
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/admin/configure.css?v='.$this->module->version
        );
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/lib/pts/tools.css?v='.$this->module->version
        );
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/lib/pts/pts-menu.css?v='.$this->module->version
        );

        //icons
        array_push(
            $this->module->params_back['CSS_FILES'],
            $this->module->path.'views/css/lib/font-awesome/font-awesome.css?v='.$this->module->version
        );

        $default_lang = Configuration::get('PS_LANG_DEFAULT');
        $iso = Language::getIsoById((int) $default_lang);
        ...
        $this->module->params_back = array_merge(array(
            'ACTIONS_CONTROLLER_URL' => $this->context->link->getAdminLink('AdminActions'.$this->module->prefix_module),
    ...
    public function readFile($module, $iso_code, $default = false)
    {
        ...
        $file_path  = _PS_THEME_DIR_.'modules/'.$this->module->name.'/translations/'.$file_name;
        $is_theme   = true;

        if (!file_exists($file_path)) {
            $file_path  = $this->module->translations_path.$file_name;
            $is_theme   = false;
        ...
        while (!feof($file)) {
            ...
            if ($is_theme) {
                $search_string = strpos($line_explode[0], '<{'.$module.'}'._THEME_NAME_.'>');
            } else {
            ...
            if (array_key_exists(1, $line_explode) && $search_string) {
                if ($is_theme) {
                    $file_md5 = str_replace("$"."_MODULE['<{".$module."}"._THEME_NAME_.">", '', $line_explode[0]);
                } else {
    ...
    public function shareTranslation($iso_code)
    {
        ...
        if (file_exists($file_path)) {
            ...
            $send = Mail::Send(
                $id_lang,
                'test',
                $this->module->l('he shared a translation with you', self::NAME_CLASS).' - PS:'._PS_VERSION_.' - MOD:'.$this->module->name.' - v'.$this->module->version,
                array(),
        ...
        if (is_writable($file_path)) {
            ...
            foreach ($data_translation as $key => $value) {
                foreach ($value as $data) {
                    ...
                    if ($is_theme) {
                        $line .= '$_MODULE[\'<{'.$this->module->name.'}'._THEME_NAME_.'>'.$key.'_';
                    } else {
            ...
            $path_file_template = dirname(__FILE__).'/../../themes/'._THEME_NAME_.'/modules/'.$this->module->name.'/translations/'.$iso_code.'.php';
