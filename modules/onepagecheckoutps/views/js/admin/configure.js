...
var AppOPC = {
    ...
    initStatistics: function() {
        if ($('div#tab-statistics #statistics').length === 0 || Object.keys(social_data).length === 0) {
            return false;
        }
        ...
        new Chart(ctxP, {
            type: 'pie',
            data: {
                labels:  Object.values(social_data.labels),
                datasets: [{
                    data:  Object.values(social_data.data),
                    backgroundColor:  Object.values(social_data.backgroundColor),
                }]
            },
            options: {
                responsive: true,
            ...
            success: function (data)
            {
                if (data.message_code === SUCCESS_CODE) {
                    ...
                    if (typeof data.name_image !== typeof undefined && !$.isEmpty(data.name_image)){
                        var d = new Date();
                        var src = module_img + 'payments/' + data.name_image + '?' + d.getTime();
