...
<div id="field_{if $field->object neq ''}{$field->object|escape:'htmlall':'UTF-8'}_{/if}{$field->name|escape:'htmlall':'UTF-8'}"
    ...
    {if $field->type_control eq $OPC_GLOBALS->type_control->textbox}
        ...
        <input
            ...
            class="{$field->classes|escape:'htmlall':'UTF-8'} form-control input-sm not_unifrom not_uniform not-styling {if $field->is_custom}custom_field{/if} {if $field->capitalize}capitalize{/if}"
        ...
        />
        {if $field->label neq ''}
            <em>{$field->label|escape:'html':'UTF-8':false:true}</em>
        {/if}
            ...
            {if isset($field->options.data)}
                {foreach from=$field->options.data item='item' name='f_options'}
                    <option
                        ...
                        data-text="{$item[$field->options.description]|escape:'htmlall':'UTF-8'}"
                        {if $field->name == 'id_country'}data-iso-code="{$item['iso_code']|escape:'htmlall':'UTF-8'}"{/if}
...
{*if $CONFIGS.OPC_AUTO_ADDRESS_GEONAMES and $field->name == 'address1'}
    <div class="form-group col-xs-12 clear clearfix inner-addon left-addon" id="field_{$field->object}_search_by_postcode">
