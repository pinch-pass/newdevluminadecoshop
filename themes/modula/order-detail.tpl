...
<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
		...
		<input type="hidden" value="" name="submitReorder"/>
			...
			<a href="#" onclick="$(this).closest('form').submit(); return false;" class="button btn btn-default button-medium pull-right"><span>{l s='Reorder'}<i class="icon-long-arrow-right right"></i></span></a>
		<p class="dark">
...
<h1 class="page-heading-on">{l s='Follow your order\'s status step-by-step'}</h1>
...
<div id="order-detail-content" class="table_block table-responsive">
	<table class="table table-bordered">
		...
		<tfoot>
			...
			<tr class="item">
				<td colspan="{if $return_allowed}2{else}1{/if}">
					<strong>{l s='Items'} {if false && $use_tax}{l s='(tax incl.)'}{/if} </strong>
				</td>
				<td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
...
{if $return_allowed}
	<div id="returnOrderMessage">
		<h3 class="page-heading-on bottom-indent">{l s='Merchandise return'}</h3>
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-default button button-medium"><span><i class="icon-refresh left"></i> {l s='Make an RMA slip'}</span></button>
...
{if !$is_guest}
	{if count($messages)}
	<h3 class="page-heading-on">{l s='Messages'}</h3>
	 <div class="table_block">
		<table class="detail_step_by_step table table-bordered">
			<thead>
				<tr>
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		<h3 class="page-heading-on bottom-indent">{l s='Add a message'}</h3>
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button btn btn-default button-medium"><span><i class="icon-envelope left"></i> {l s='Send'}</span></button>
		</div>
	</form>
{else}
