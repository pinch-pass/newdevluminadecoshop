			...
			<div class="main-categories__grid">
				{foreach name=items from=$htmlitems item=hItem}
					<div class="main-categories__box">
						{if $hItem.image}
							<img src="{$link->getMediaLink("`$module_dir`img/`$hItem.image`")}" alt="{$hItem.title|escape:'htmlall':'UTF-8'}" class="main-categories__img">
						{/if}
						{if $hItem.title}
							<div class="main-categories__title">{$hItem.title|escape:'htmlall':'UTF-8'}</div>
						{/if}
						{if $hItem.url}
							<a href="{$hItem.url|escape:'htmlall':'UTF-8'}" class="main-categories__link">{$hItem.title|escape:'htmlall':'UTF-8'}</a>
						{/if}
					</div>
					{*<li class="htmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'} col-xs-4">
						...
						{/if}
							{if $hItem.image}
								<img src="{$link->getMediaLink("`$module_dir`img/`$hItem.image`")}" class="item-img" title="{$hItem.title|escape:'htmlall':'UTF-8'}" alt="{$hItem.title|escape:'htmlall':'UTF-8'}" width="{if $hItem.image_w}{$hItem.image_w|intval}{else}100%{/if}" height="100%" />
							{/if}
