...
{if isset($nbProducts) && $nbProducts <= 0}
	<p class="alert alert-warning"><i class="icon-warning-sign left"></i> {l s='Your shopping cart is empty.' mod='cheque'}</p>
{else}
   
	<form action="{$link->getModuleLink('cheque', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
		<div class="box cheque-box animated pulse" style="visibility: visible;">
		...
		<p class="cart_navigation clearfix" id="cart_navigation">
			<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" class="button-exclusive btn btn-default" style="float:left">
				<i class="icon-long-arrow-left left"></i> {l s='Other payment methods' mod='cheque'}
			</a>
			<button type="submit" class="btn btn-default button button-menu-white" style="float:right">
				<span><i class="icon-shopping-cart left"></i> {l s='I confirm my order' mod='cheque'}</span>
			</button>
		</p>
	</form>
{/if}
