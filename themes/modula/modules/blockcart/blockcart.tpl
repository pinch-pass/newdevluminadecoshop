...
{if isset($blockcart_top) && $blockcart_top && $blockcart_nav|intval != 1}
<div class="col-sm-cart clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}

	{if $blockcart_nav|intval != 1}
	<div class="shopping_cart">
		<a class="cart-icon" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
		
		<img src="{$img_dir}shopping-cart.svg" alt="{l s='Корзина' mod='blockcart'}" />
		<img class="hover-link " src="{$img_dir}shopping-cart-hov.svg" alt="{l s='Корзина' mod='blockcart'}" />
		{l s='Корзина' mod='blockcart'}
		...
		{if !$PS_CATALOG_MODE}
            ...
            <span class="cart-arrow"> </span>
                ...
                <p class="cart-subtitle">{l s='Recently added' mod='blockcart'}</p>
							...
							<dl class="products">
								{foreach from=$products item='product' name='myLoop'}
									...
									<dt data-id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										<a class="cart-images" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" ><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default')}" alt="{$product.name|escape:'html':'UTF-8'}" /></a>
										<div class="cart-info">
											<div class="product-name">
												<span class="quantity-formated"><span class="quantity">{$product.cart_quantity}</span>&nbsp;x&nbsp;</span><a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" >{$product.name|truncate:23:' ...'|escape:'html':'UTF-8'}</a>
											</div>
...
{if isset($customizedDatas.$productId.$productAttributeId[$product.id_address_delivery])}
						...
						<div class="cart-prices">
							{* <div class="cart-prices-line first-line">
								...
								<span>
									{l s='Shipping' mod='blockcart'}:&nbsp
								</span>
							</div> *}
							{if $show_wrapping}
							...
							<div class="cart-prices-line last-line">
								...
								<span>{l s='Total' mod='blockcart'}&nbsp</span>
							</div>
							{if false && $use_taxes && $display_tax_label == 1 && $show_tax}
								<p>
								{if $priceDisplay == 0}
						...
						<p class="cart-buttons">
							<a id="button_order_cart" class="btn btn-default button button-small" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{*{l s='Check out' mod='blockcart'}*}" rel="nofollow">
								<span>
									{l s='Check out' mod='blockcart'} <i class="icon-long-arrow-right right"></i>
								</span>
							</a>
						</p>
					</div>
...
{if isset($blockcart_nav) && $blockcart_nav} 
				...
				<!-- Singular Case [both cases are needed because page may be updated in Javascript] -->
								...
								<div class="button-container">	
									<span class="continue btn btn-default button button-medium">
										<span>
											<i class="icon-long-arrow-left"></i>&nbsp; {l s='Continue shopping' mod='blockcart'}
										</span>
									</span>
									<a class="btn btn-default button button-medium"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" rel="nofollow">
										<span>
											<i class="icon-shopping-cart"></i>&nbsp; {l s='Proceed to checkout' mod='blockcart'}
										</span>
									</a>
								</div>
							</div>
