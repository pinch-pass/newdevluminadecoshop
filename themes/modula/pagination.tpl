...
{if isset($smarty.get.n) && $smarty.get.n}{$products_per_page = $smarty.get.n|intval}{/if}
{$pages_nb = ($nb_products / $products_per_page)|ceil}
{if $stop > $pages_nb}{$stop = $pages_nb}{/if}
{if $start!=$stop}
<ul class="pagination">
{if $p != 1}
...
<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous">
						<a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">
							<span>
							<i class="icon-long-arrow-left left"></i>&nbsp;&nbsp;{l s='Previous'}
							</span>
						</a>
					</li>
				{else}
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next">
						<a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">
							<span>
								{l s='Next'}&nbsp;&nbsp;<i class="icon-long-arrow-right right"></i>
							</span>
						</a>
					</li>
				{else}
