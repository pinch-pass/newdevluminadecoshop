...
{if isset($empty)}
	<p class="alert alert-warning"><i class="icon-warning-sign left"></i> {l s='Your shopping cart is empty.'}</p>
{elseif $PS_CATALOG_MODE}
	<p class="alert alert-warning"><i class="icon-warning-sign left"></i> {l s='This store has not accepted your new order.'}</p>
{else}
	<p style="display:none" id="emptyCartWarning" class="alert alert-warning"><i class="icon-warning-sign left"></i> {l s='Your shopping cart is empty.'}</p>
...
{if isset($lastProductAdded.attributes) && $lastProductAdded.attributes}
	...
	<div id="order-detail-content" class="table_block table-responsive">
		<table id="cart_summary" class="table table-bordered animated pulse" style="visibility: visible;">
			<thead>
				<tr>
				...
				{if $use_taxes}
					{if $priceDisplay}
						<tr class="cart_total_price">
							...
							<td colspan="3" class="text-right">{if false && $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products'}{/if}</td>
					...
					{else}
						<tr class="cart_total_price">
							...
							<td colspan="3" class="text-right">{if false && $display_tax_label}{l s='Total products (tax incl.)'}{else}{l s='Total products'}{/if}</td>
					...
					{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
						{if $priceDisplay}
							<tr class="cart_total_delivery" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
								<td colspan="3" class="text-right">{if false && $display_tax_label}{l s='Total shipping (tax excl.)'}{else}{l s='Total shipping'}{/if}</td>
						...
						{else}
							<tr class="cart_total_delivery"{if $total_shipping <= 0} style="display:none;"{/if}>
								<td colspan="3" class="text-right">{if false && $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}</td>
			...
			{if sizeof($discounts)}
                                {foreach $discounts as $discount}
                                    {if !in_array($discount.id_cart_rule,[1,2,3])}
                                        <tbody>
	...
	{if ((!empty($delivery_option) AND !isset($virtualCart)) OR $delivery->id OR $invoice->id) AND !$opc}
		<div class="order_delivery clearfix row wow animated fadeInLeft" style="visibility: visible;">
{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
				{if $delivery->id}
					<div class="{if $delivery->alias !== $invoice->alias}class="col-xs-12 col-sm-6"{else}class="col-xs-12 col-sm-12"{/if}"{if !$have_non_virtual_products} style="display: none;"{/if}>
				...
				{/if}
				{if $invoice->id}
					<div class="col-xs-12 col-sm-6" {if $delivery->alias === $invoice->alias}style="display:none;"{/if}>
	...
	<p class="cart_navigation clearfix">
		...
		{if !$opc}
			<a
				...
				class="btn btn-default button button-menu-white"
				style="float:right">
				<span><i class="icon-shopping-cart left"></i> {l s='Proceed to checkout'}</span>
			</a>
		{/if}
