	...
	{foreach from=$multipleAddresses item=address name=myLoop}
    	<div class="col-xs-12 col-sm-6 address">
			<ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if} box">
                ...
                <a class="btn btn-default button button-medium" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}" title="{l s='Update'}"><span><i class="icon-download-alt left"></i> {l s='Update'}</span></a>
                <a class="btn btn-default button button-medium" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}"><span><i class="icon-trash left"></i> {l s='Delete'}</span></a></li>
            </ul>
        </div>
	{/foreach}
	</div>
...
<a href="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" title="{l s='Add an address'}" class="btn btn-default button button-medium"><span>{l s='Add a new address'}<i class="icon-plus right"></i></span></a>

</div>

<ul class="footer_links clearfix">

<li><a class="btn btn-defaul button button-medium" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}"><span><i class="icon-long-arrow-left left"></i> {l s='Back to your account'}</span></a></li>

<li><a class="btn btn-defaul button button-medium" href="{$base_dir}"><span><i class="icon-long-arrow-left left"></i> {l s='Home'}</span></a></li>
</ul>