...
{else}
	<div id="carrier_area" class="opc-main-block">
		<h1 class="page-heading-on step-num"><span class="heading-number">2</span> Доставка</h1>
			<div id="opc_delivery_methods" class="opc-main-block">
				<div id="opc_delivery_methods-overlay" class="opc-overlay" style="display: none;"></div>
{/if}
<div class="order_carrier_content box animated pulse" style="visibility: visible;">
		...
		{if !$opc}
				<p class="cart_navigation clearfix">
					...
					{if !$is_guest}
						{if $back}
							<a 
								...
								title="{l s='Previous'}"
								class="button-exclusive btn btn-default" style="float:left">
					...
					{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
						<button type="submit" name="processCarrier" class="btn btn-default button button-menu-white" style="float:right">
							<span>
								<i class="icon-shopping-cart left"></i> {l s='Continue'} 
	...
	<div id="ordermsg" class="form-group">
			<label>Если вы хотите добавить комментарий о вашем заказе, пожалуйста, напишите его в поле ниже.</label>
			<textarea class="form-control" cols="60" rows="6" name="message" id="dost_mes">{if isset($oldMessage)}{$oldMessage}{/if}</textarea>
		</div>
		</div> <!-- end opc_delivery_methods -->
		<script>
        $('#dost_mes').on('keyup', function(e) {
           var messa = $('#dost_mes').val();
          $('#other').val(messa);
        })
    </script>
	{/if}
