...
<div class="box">
	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std" id="add_address">
		...
		<div class="required form-group" id="adress_alias" style="display:none">
			...
			<input type="hidden" id="alias" class="is_required validate form-control" data-validate="{$address_validation.alias.validate}" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{else if isset($address->alias)}{$address->alias|escape:'html':'UTF-8'}{elseif !$select_address}{l s='My address'}{/if}" />
		</div>
		<p class="submit2">
			...
			<button type="submit" name="submitAddress" id="submitAddress" class="btn btn-default button button-medium">
				<span>
                <i class="icon-download-alt left"></i>
					{l s='Save'}
...
<ul class="footer_links clearfix">
	<li>
		<a class="btn btn-defaul button button-medium" href="{if isset($smarty.get.back)}zamowienie?step=1{else}{$link->getPageLink('addresses', true)|escape:'html':'UTF-8'}{/if}">
			<span><i class="icon-long-arrow-left left"></i> {l s='Back to your addresses'}</span>
		</a>
	</li>
</ul>
{strip}
