...
{else}
	...
	<h1 class="page-heading-on step-num"><span class="heading-number">1</span> {l s='Addresses'}</h1>
	<div id="opc_account" class="opc-main-block">
		<div id="opc_account-overlay" class="opc-overlay" style="display: none;"></div>
{/if}
<div class="addresses clearfix">
	<div class="row wow animated fadeInRight" style="display: none;">
		...
		<div class="col-xs-12 col-sm-6">
			<div id="address_invoice_form" class="select form-group selector1"{if $cart->id_address_invoice == $cart->id_address_delivery} style="display: none;"{/if}>
				...
				{else}
					<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" class="button button-medium btn btn-default">
						<span>
							{l s='Add a new address'} <i class="icon-plus right"></i>
						</span>
					</a>
				{/if}
			</div>
	...
	<div class="row wow animated fadeInLeft" style="visibility: visible;">
		<div {if $cart->id_address_invoice != $cart->id_address_delivery}class="col-xs-12 col-sm-6"{else}class="col-xs-12 col-sm-12"{/if}{if $cart->isVirtualCart()} style="display:none;"{/if}>
			<ul class="address item box" id="address_delivery">
			</ul>
		</div>
		<div class="col-xs-12 col-sm-6" {if $cart->id_address_invoice == $cart->id_address_delivery}style="display:none;"{/if}>
			<ul class="address alternate_item{if $cart->isVirtualCart()} full_width{/if} box" id="address_invoice">
			</ul>
		</div>
	</div> <!-- end row -->
	<p class="address_add submit" style="display:none;">
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" class="button button-medium btn btn-default">
			<span>{l s='Add a new address'} <i class="icon-plus right"></i></span>
		</a>
	</p>
	{if !$opc}
		<div id="ordermsg" class="form-group wow animated fadeInUp" style="visibility: visible;">
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html':'UTF-8'}" class="button-exclusive btn btn-default" style="float:left">
					<i class="icon-long-arrow-left left"></i> {l s='Continue Shopping'}
				</a>
				<button type="submit" name="processAddress" class="btn btn-default button button-menu-white" style="float:right">
					<i class="icon-shopping-cart left"></i> <span>{l s='Continue'} </span>
				</button>
			</p>
		</form>
{else}
...
{capture}<a class="button button-medium btn btn-default" href="{$smarty.capture.addressUrlAdd}"><span><i class="icon-download-alt left"></i> {l s='Update' js=1}</span></a>{/capture}
