				...
				{foreach from=$orders item=order name=myLoop}
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						<td class="history_detail">
							<a class="btn btn-default button button-medium" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
								<span>
									<i class="icon-reorder left"></i> {l s='Details'}
								</span>
							</a>
							{if isset($opc) && $opc}
...
<ul class="footer_links clearfix">
	<li>
		<a class="btn btn-default button button-medium" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
			<span>
				<i class="icon-long-arrow-left lef"></i>&nbsp; {l s='Back to Your Account'}
			</span>
		</a>
	</li>
	<li>
		<a class="btn btn-default button button-mefium" href="{$base_dir}">
			<span><i class="icon-long-arrow-left left"></i>&nbsp; {l s='Home'}</span>
		</a>
	</li>
</ul>