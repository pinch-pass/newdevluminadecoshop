...
function ddd($var, $if = true)
{
    if (!$if) return;
    if (in_array($_SERVER['REMOTE_ADDR'], array('109.197.193.177', '176.116.137.9'))) {
        $_ = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 1);
        echo '<pre>'.print_r($var, true).'</pre>';
        die($_[0]['file'].' in LINE: '.$_[0]['line']);
    }
}
